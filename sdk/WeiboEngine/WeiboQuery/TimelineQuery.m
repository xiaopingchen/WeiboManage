...
- (NSString *)apiUrl:(StatusTimeline)timeline {
    ...
    else if (timeline==StatusUserTimeline) {
...
-(void)queryUserTimelineWithUid:(NSString *)uid sinceId:(long long)sinceId  count:(int)count{
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   [NSString stringWithFormat:@"%lld", sinceId], @"since_id",                                   
                                   [NSString stringWithFormat:@"%d", count], @"count",
                                   uid,@"uid",
                                    nil];
    [super getWithAPIPath:[self apiUrl:StatusUserTimeline]
                   params:params];
}
